...
bool isProcessExist(LPCSTR szIn,LPDWORD lpdwProcessIdIn=NULL){
	...
	bool HasMore=Process32First(hdSnap,&pe32);
	while (HasMore)
	{
		if (!strcmp(pe32.szExeFile,szIn))
			if (lpdwProcessIdIn!=NULL && pe32.th32ProcessID!=0)
				return (bool)(*lpdwProcessIdIn=pe32.th32ProcessID);
			else
				return true;
		HasMore=Process32Next(hdSnap,&pe32);
	}
	return false;
}
...
DWORD WINAPI GuardThread(LPVOID){
	...
	while (1){
		if (isProcessExist(_szProcName,&dwProcessId)){
//			Sleep(5*600000);
			Func_FastPMNTS("fuck");
